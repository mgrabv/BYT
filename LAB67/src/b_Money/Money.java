...
public class Money implements Comparable {
	...
	public Money add(Money other) {
		if (other.currency.equals(currency)) {
			...
			return new Money(amount + (int)(other.universalValue() / currency.getRate()), currency);
		}
	}
	 ...
	 */
	public Money negate() {
		return new Money(amount - (2 * amount), currency);
	}
	...
	public int compareTo(Object other) {
		return this.universalValue().equals(((Money)other).universalValue()) ? 0 : this.universalValue() > (((Money)other).universalValue()) ? 1 : -1;
	}
}